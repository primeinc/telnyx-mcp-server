...
class TestOAuthFlow:
    ...
    def test_oauth_callback_success(self, client):
        ...
        respx.post("https://login.microsoftonline.com/test_tenant_id/oauth2/v2.0/token").mock(
...
class TestMCPProtocol:
    ...
    def test_mcp_tools_call_with_auth(self, mock_call_tool, mock_verify, client, auth_headers):
...
class TestSSEStreaming:
    ...
    def test_sse_with_auth(self, mock_verify, client, auth_headers):
        ...
        assert response.status_code in [200, 406]  # 406 if method not allowed is returned
...
class TestErrorHandling:
    ...
    @patch('telnyx_mcp_server.remote.auth.verify_jwt_token')
    @patch('telnyx_mcp_server.mcp.call_tool')
    def test_tool_execution_error(self, mock_call_tool, mock_verify, client, auth_headers):
