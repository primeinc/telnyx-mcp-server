...
class PIIRedactorProcessor:
    ...
    SENSITIVE_PATTERNS = {
        'token': re.compile(r'(bearer\s+|token["\s]*[:=]\s*["\']?)([a-zA-Z0-9_\-\.]{20,})', re.IGNORECASE),
        'authorization': re.compile(r'(authorization["\s]*[:=]\s*["\']?)([a-zA-Z0-9_\-\.\/\+]{20,})', re.IGNORECASE),
        'api_key': re.compile(r'(api[_\s]*key["\s]*[:=]\s*["\']?)([a-zA-Z0-9_\-]{20,})', re.IGNORECASE),
        'password': re.compile(r'(password["\s]*[:=]\s*["\']?)([^"\s]{3,})', re.IGNORECASE),
        'secret': re.compile(r'(secret["\s]*[:=]\s*["\']?)([a-zA-Z0-9_\-]{10,})', re.IGNORECASE),
    ...
    SENSITIVE_KEYS = {
        ...
        'authorization', 'auth', 'bearer', 'api_key', 'apikey', 'key',
        'password', 'passwd', 'pwd', 'secret', 'client_secret',
        'user_data', 'personal_info', 'pii', 'email', 'mail',
        'phone', 'telephone', 'mobile', 'ssn', 'social_security',
        'credit_card', 'cc', 'card_number', 'account_number'
    }
    ...
    def _redact_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(sensitive_key in redacted_key for sensitive_key in self.SENSITIVE_KEYS):
    ...
    def _redact_string(self, text: str) -> str:
        ...
        for pattern_name, pattern in self.SENSITIVE_PATTERNS.items():
            if pattern_name in ['token', 'authorization', 'api_key', 'password', 'secret']:
                # For tokens and keys, keep the prefix but redact the value
                redacted_text = pattern.sub(r'\1' + self.redaction_string, redacted_text)
            else:
                # For other patterns like email, phone, redact entirely
                redacted_text = pattern.sub(self.redaction_string, redacted_text)
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            for name, value in headers.items():
                if name.decode().lower() in ['x-trace-id', 'x-request-id', 'mcp-session-id']:
...
def configure_structlog(
    ...
    if application_insights_key:
        try:
            ...
            logging.warning("Application Insights requested but opencensus-ext-azure not available")
