...
with patch.dict('sys.modules', {'structlog': structlog_mock}):
    from telnyx_mcp_server.remote.structured_logging import (
        PIIRedactorProcessor, add_trace_id, add_timestamp, add_log_level,
        add_logger_name, configure_structlog, get_logger, set_trace_id,
        get_trace_id, clear_trace_id
    )
...
class TestConfigureStructlog:
    ...
    def test_configure_structlog_with_structlog_available(self):
        ...
        with patch('telnyx_mcp_server.remote.structured_logging.STRUCTLOG_AVAILABLE', True):
            configure_structlog(
                log_level="INFO",
    ...
    def test_configure_structlog_without_structlog(self):
        ...
        with patch('telnyx_mcp_server.remote.structured_logging.STRUCTLOG_AVAILABLE', False):
...
class TestTraceIdMiddleware:
    ...
    async def test_trace_id_middleware_generates_id(self):
        ...
        from telnyx_mcp_server.remote.structured_logging import TraceIdMiddleware
    ...
    async def test_trace_id_middleware_uses_existing_id(self):
        ...
        scope = {
            "type": "http",
            "headers": [
                (b"x-trace-id", b"existing_trace_123")
            ]
        }
