...
class AsyncRedisAuthStore:
    ...
    async def create_auth_code(
        ...
        success = await self._store_in_redis(
            key, auth_data.to_dict(), self.code_ttl
        )
        ...
        if not success:
            ...
            logger.info(
                f"Stored auth code in fallback store for user: {user_info.get('email', 'unknown')}"
            )
        else:
            logger.info(
                f"Stored auth code in Redis for user: {user_info.get('email', 'unknown')}"
            )
        ...
        if not data_dict:
            ...
            if fallback_entry:
                ...
                if time.time() > fallback_entry["expires_at"]:
                    ...
                    logger.warning(
                        f"Auth code expired in fallback store: {code[:8]}..."
                    )
                    return None
    ...
    async def mark_code_used(self, code: str) -> bool:
        ...
        if data_dict:
            ...
            if success:
                logger.info(
                    f"Marked auth code as used in Redis: {code[:8]}..."
                )
                return True
        ...
        if fallback_entry:
            ...
            logger.info(
                f"Marked auth code as used in fallback store: {code[:8]}..."
            )
            return True
    ...
    async def create_session(
        ...
        if not success:
            ...
            logger.info(
                f"Stored session in fallback store: {session_id[:8]}..."
            )
        else:
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        if session and session.state:
            fallback_state_key = self._fallback_store_key(
                f"{self.STATE_INDEX_PREFIX}{session.state}"
            )
    ...
    async def cleanup_expired(self) -> int:
        ...
        if expired_keys:
            logger.info(
                f"Cleaned up {len(expired_keys)} expired entries from fallback store"
            )
