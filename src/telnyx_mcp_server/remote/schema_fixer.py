...
class MessageToolSchema(BaseModel):
    ...
    messaging_profile_id: Optional[str] = Field(
        None, description="Messaging profile ID"
    )
    subject: Optional[str] = Field(None, description="Message subject")
    media_urls: Optional[List[str]] = Field(
        None, description="List of media URLs"
    )
    webhook_url: Optional[str] = Field(None, description="Webhook URL")
    type: Optional[str] = Field(
        None, description="The protocol for sending the message"
    )
...
class PhoneNumberListSchema(BaseModel):
    ...
    filter_phone_number: Optional[str] = Field(
        None, description="Filter by phone number"
    )
    filter_status: Optional[str] = Field(None, description="Filter by status")
    filter_voice_enabled: Optional[bool] = Field(
        None, description="Filter by voice enabled"
    )
...
class CloudStorageBucketSchema(BaseModel):
    ...
    region: Optional[str] = Field(
        None, description="Region to create the bucket in"
    )
...
class CloudStorageFileSchema(BaseModel):
    ...
    local_path: Optional[str] = Field(
        None, description="Local path for file operations"
    )
...
class IntegrationSecretCreateSchema(BaseModel):
    ...
    token: Optional[str] = Field(
        None, description="The token for the secret (required for bearer type)"
    )
    username: Optional[str] = Field(
        None,
        description="The username for the secret (required for basic type)",
    )
    password: Optional[str] = Field(
        None,
        description="The password for the secret (required for basic type)",
    )
...
class IntegrationSecretListSchema(BaseModel):
    ...
    filter_type: Optional[str] = Field(
        None, description="Filter by secret type (bearer, basic)"
    )
...
def extract_docstring_parameters(docstring: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if in_args and line and ":" in line:
            ...
            is_required = (
                "Required." in description or "required." in description
            )
            is_optional = (
            ...
            if (
                ...
                "array" in description.lower() or "list" in description.lower()
            ):
                param_type = "array"
            elif (
...
def validate_tool_arguments(
    ...
    try:
        ...
        if "from" in arguments and tool_name in [
            "send_message",
...
def fix_tool_schema(tool: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if tool_name in TOOL_SCHEMAS:
        ...
        try:
            ...
            logger.warning(
                f"Failed to apply Pydantic schema for {tool_name}: {e}"
            )
    ...
    if len(properties) == 1 and "request" in properties:
        ...
        if flattened_schema.get("properties"):
            ...
            logger.debug(
                f"Flattened schema from docstring for tool: {tool_name}"
            )
            return updated_tool
...
def register_tool_schema(
    tool_name: str, schema_model: Type[BaseModel]
) -> None:
