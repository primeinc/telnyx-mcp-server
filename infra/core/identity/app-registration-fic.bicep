...
@description('Specifies the ID of the App Service managed identity.')
...
resource clientApp 'Microsoft.Graph/applications@v1.0' = {
  ...
  // Federated Identity Credential for App Service managed identity
  resource clientAppFic 'federatedIdentityCredentials@v1.0' = {
    name: '${clientApp.uniqueName}/appServiceFic'
    audiences: [
      audiences[cloudEnvironment].uri
   ]
    issuer: issuer
    subject: webAppIdentityId
    description: 'App Service managed identity federated credential'
  }
}
