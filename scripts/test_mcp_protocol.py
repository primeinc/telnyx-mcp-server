    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Token endpoint auth method: {client_info.get('token_endpoint_auth_method')}"
                )
...
async def test_mcp_initialize(token: Optional[str] = None):
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{BASE_URL}/mcp", json=message, headers=headers
            )
            print(f"Status: {response.status_code}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for result in results:
                    ...
                    print(
                        f"  - {method}: {len(result['result'].get(method, []))} items"
                    )
                return True
            else:
...
async def test_mcp_sse_support(token: Optional[str] = None):
    ...
    message = {"jsonrpc": "2.0", "id": 5, "method": "tools/list", "params": {}}
