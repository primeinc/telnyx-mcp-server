...
async def optional_auth(
    ...
    logger.info(
        f"optional_auth called - credentials present: {credentials is not None}"
    )
    ...
    if credentials:
        try:
            token = credentials.credentials
            logger.info(
                f"Received token (first 20 chars): {token[:20] if len(token) >= 20 else token}..."
            )
            ...
            logger.error(
                f"Token validation failed: {type(e).__name__}: {str(e)}"
            )
            import traceback
